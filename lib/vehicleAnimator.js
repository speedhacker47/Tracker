...
// DeadReckoningEngine is imported lazily inside the constructor to avoid
// it being included in any SSR bundle by Next.js's static analyser.
// This file is only ever loaded via dynamic import() inside a useEffect,
// but the static analyser still sees top-level imports.
let DeadReckoningEngine;
if (typeof window !== 'undefined') {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    DeadReckoningEngine = require('@/lib/deadReckoning').DeadReckoningEngine;
}
...
export class VehicleAnimator {
    ...
    constructor(updateCallback) {
        ...
        this._dr = DeadReckoningEngine ? new DeadReckoningEngine() : null;
    ...
    onNewPosition(deviceId, position) {
        ...
        if (this._dr) this._dr.updatePosition(deviceId, { lat, lng, speed, course, timestamp });
    ...
    removeDevice(deviceId) {
        ...
        if (this._dr) this._dr.removeDevice(deviceId);
    }
    ...
    _updateDevice(deviceId, state, now) {
        ...
        if (state.isStopped) {
            ...
            const predicted = this._dr ? this._dr.getPredictedPosition(deviceId) : null;
            if (predicted) {
                lat = predicted.lat;
                lng = predicted.lng;
